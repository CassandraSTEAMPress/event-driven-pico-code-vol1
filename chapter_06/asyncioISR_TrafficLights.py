"""
asyncioISR_TrafficLights.py
--------
Pelican LED Traffic Lights with active Piezo crosswalk buzzer and
    push button using ISR and asyncio

Traffic Light Logic Flow::
  No Pedestrian:
      Red -> Green, Green -> Yellow -> Red
  Pedestrian Crossing:
      Setting the flag variable button_pressed = True indicates
          that the crosswalk button was pressed 
      On next Red, buzz for 5 seconds allowing pedestrian to 
          cross then set button_pressed = False and proceed to 
...
async def traffic_lights(light_duration = 4): 
    """Simulate a traffic light using asyncio"""
...
async def main():
    ...
    await asyncio.gather(button_task, traffic_controller_task)
...
if __name__ == "__main__":
    ...
    # Turn off LEDs
        ...
        # Cleanup (e.g., disable IRQ) if necessary
        crosswalk_button.irq(handler=None)  # Disable interrupt
    
        print("\nAsyncio Pelican Traffic Light Controller "
              "program ended.")